...
def get_script_directory() -> Path:
	f = sys.executable if getattr(sys, "frozen", False) else __file__
...
def process_file(
	...
	with tempfile.TemporaryDirectory() as temp_dir:
		...
		decode_audio_to_wav(
			...
			output_wav_path=temp_input_wav,
			target_sample_rate=cfg.getint("basic", "target_sample_rate")
		)
		...
		x = apply_tv_eq(
			x,
			sample_rate=sr,
			band_low_hz=cfg.getfloat("tv_sound", "band_low_hz"),
			band_high_hz=cfg.getfloat("tv_sound", "band_high_hz"),
			low_mid_split_hz=cfg.getfloat("tv_sound", "low_mid_split_hz"),
			mid_high_split_hz=cfg.getfloat("tv_sound", "mid_high_split_hz"),
			low_gain_db=cfg.getfloat("tv_sound", "low_gain_db"),
			mid_gain_db=cfg.getfloat("tv_sound", "mid_gain_db"),
			high_gain_db=cfg.getfloat("tv_sound", "high_gain_db"),
		)

		print("Applying distortion...")
		x = apply_soft_clip_distortion(
			x,
			drive_db=cfg.getfloat("distortion", "drive_db"),
			mix=cfg.getfloat("distortion", "mix"),
		)

		print("Adding hiss...")
		x = add_hiss(x, level_db_rel=cfg.getfloat("noise", "hiss_level_db_rel"))

		print("Adding hum...")
		x = add_hum(
			x,
			sample_rate=sr,
			level_db_rel=cfg.getfloat("noise", "hum_level_db_rel"),
			base_freq_hz=cfg.getfloat("noise", "hum_frequency_hz"),
			harmonics=cfg.getint("noise", "hum_harmonics"),
		)

		print("Normalizing...")
		x = normalize_peak(x, cfg.getfloat("tv_sound", "output_gain_db"))
...
def main() -> None:
	...
	input_path = script_dir / cfg["basic"]["input_filename"]
	output_path = script_dir / cfg["basic"]["output_filename"]
